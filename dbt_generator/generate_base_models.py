...
def generate_base_model(table_name, source_name, case_sensitive, leading_commas):
	print(f'Generating base model for table {table_name}')
	bash_command = f'''
		dbt run-operation generate_base_model --args \'{{"source_name": "{source_name}", "table_name": "{table_name}", "case_sensitive_cols": {case_sensitive}, "leading_commas": {leading_commas}}}\'
	'''
	if system() == 'Windows':
